...
const salesReport = async (req, res, next) => {
    ...
    try {
       ...
       const orders = await Order.find({ orderDate: { $gte: from, $lte: to }, orderStatus: 'Delivered' }).populate('userId');
...
const adminDownloadReports = async (req, res, next) => {
    try {
        ...
        let { from, to } = req.query
        to += 'T23:59:59.999Z'
        const orders = await Order.find({ orderDate: { $gte: from, $lte: to }, orderStatus: 'Delivered' })
            .populate('userId')
            .populate({
                path: 'products.productId',
                model: 'Product',
            });
        const netTotalAmount = orders.reduce((acc, order) => acc + order.totalprice, 0)
